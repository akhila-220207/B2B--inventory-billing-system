...
export default function BuyerOverview() {
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
           <div className="text-center md:text-left">
              ...
              <div className="mt-8 flex flex-wrap gap-4 justify-center md:justify-start">
                 ...
                 >
                    <FaRocket className="text-blue-400" /> Start Shopping
                 </button>
              </div>
           </div>
           ...
           <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-50 py-6 px-8 rounded-3xl border border-slate-100/50">
                 <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mb-1">Total Spent</p>
                 <p className="text-xl font-black text-slate-900">₹{stats.spending.toLocaleString("en-IN")}</p>
              </div>
              <div className="bg-slate-50 py-6 px-8 rounded-3xl border border-slate-100/50">
      ...
      {/* Quick Access Area */}
      <div>
        <h3 className="text-[10px] text-slate-400 font-black uppercase tracking-[0.3em] mb-8 ml-2">Quick Links</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
           {quickLinks.map((link, i) => (
             <Link 
               key={i} 
               to={link.to} 
               className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-lg shadow-slate-200/20 hover:shadow-2xl hover:-translate-y-1 transition-all group"
             >
                <div className={`w-14 h-14 ${link.color} rounded-2xl flex items-center justify-center text-white text-xl mb-6 shadow-lg group-hover:scale-110 transition-transform`}>
                   <link.icon />
                </div>
                <h4 className="text-sm font-black text-slate-900 mb-1">{link.label}</h4>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wide flex items-center gap-2">
                   {link.desc} <FaArrowRight className="opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all" />
                </p>
             </Link>
           ))}
        </div>
      ...
      <div className="p-8 bg-blue-600 rounded-[2.5rem] text-white flex items-center justify-between gap-8 shadow-2xl shadow-blue-200">
         <div className="flex items-center gap-6">
            ...
            <div>
               <p className="text-xs font-black uppercase tracking-widest text-blue-100 mb-1">Helpful Tip</p>
